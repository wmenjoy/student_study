...
export default function SentenceOrderPage() {
  ...
  return (
    <LessonRunner title="句子重排" skillId="cn-sentence-order" intro={{ story: "用词语块拼出完整句子。", goal: "会按语序重排句子", steps: steps }} hints={{ build: ["点击词语块加入句子"], map: ["读出完整句子"], review: ["换一个句子再试"] }} variantGen={(diff)=>{
      ...
      else if(diff==="medium"){items.push({prompt:"把'快乐地'换成'高兴地'后写出句子",placeholder:"输入句子",check:x=>x.replace(/ /g,"")===target.replace("快乐地","高兴地")})}
      else {items.push({prompt:"在句末加'。'写出句子",placeholder:"输入句子",check:x=>x.replace(/ /g,"")===target+"。"})}
      return items
