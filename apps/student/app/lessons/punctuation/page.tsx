...
export default function PunctuationPage() {
  ...
  return (
    <LessonRunner title="标点练习" skillId="cn-punctuation" intro={{ story: "给句子加上合适的标点。", goal: "会用逗号句号等标点", steps: steps }} hints={{ build: ["观察停顿位置"], map: ["选择标点并读出句子"], review: ["换一个句子再试"] }} variantGen={(diff)=>{
      const make=(t:string,a:string[])=>({label:t.replaceAll("_","□"),apply:()=>{setText(t);setAnswers(a)}})
      ...
      else if(diff==="medium"){items.push({prompt:"把最后一个句号改成惊叹号写出来",placeholder:"输入句子",check:x=>x.replaceAll(" ","")===target.replace(/。$/,"！")})}
      else {items.push({prompt:"把第一个逗号改成分号写出来",placeholder:"输入句子",check:x=>x.replaceAll(" ","")===target.replace("，","；")})}
      return items
