...
export default function AgePage() {
  ...
  return (
    <LessonRunner title="公式法：年龄问题" skillId="math-age" intro={{ story: "两人的年龄有和与差，几年后的年龄如何？", goal: "会用和差公式求年龄并推算", steps }} hints={ageScript.hints} variantGen={(difficulty)=>{
      const make=(s:number,d:number,y:number)=>({label:`和${s} 差${d} 过${y}年`,apply:()=>{setSum(s);setDiff(d);setYears(y);setStage(0)}})
      if(difficulty==="easy") return [make(20,4,5),make(30,6,5)]
      if(difficulty==="medium") return [make(40,8,10),make(36,2,6)]
