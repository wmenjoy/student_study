...
export default function AIGeneratorPage() {
  ...
  const handleSubmitAnswer = () => {
    ...
    if (isCorrect) {
      ...
      speak("再想想哦！正确答案是" + currentQuestion?.answer)
    }
  ...
  const getExplanationSteps = (): string[] => {
    ...
    // 如果没有步骤，至少显示答案
    if (steps.length === 1) {
      steps.push(`正确答案：${currentQuestion.answer}`)
      if (currentQuestion.explain) {
        steps.push(currentQuestion.explain)
      }
    }
            ...
            {questions.length === 0 ? (
              ...
              <div className="bg-white rounded-3xl shadow-2xl p-8 border-4 border-pink-200">
                ...
                <div className="mb-6 space-y-4">
                  {/* Question with Narration */}
                  <Narration avatar="/icons/area.svg" name="AI老师">
                    <div className="text-sm text-blue-600 font-bold mb-2">
                      📚 {currentQuestion.category}
                    </div>
                    <div className="text-lg font-medium leading-relaxed">{currentQuestion.prompt}</div>
                  </Narration>
                  ...
                  {!showResult && (
                    <div className="mb-4">
                      ...
                      <input
                        ...
                        className="w-full border-3 border-orange-200 rounded-2xl px-5 py-4 text-lg focus:border-orange-400 focus:ring-4 focus:ring-orange-100 outline-none shadow-md transition-all"
                        autoFocus
                      />
                    </div>
                  )}
                    ...
                    <>
                      <div className={`p-6 rounded-2xl shadow-lg ${isCorrect ? 'bg-gradient-to-r from-green-50 to-emerald-50 border-3 border-green-300' : 'bg-gradient-to-r from-red-50 to-pink-50 border-3 border-red-300'}`}>
                        ...
                        <div className="bg-white/70 p-4 rounded-xl mb-3 shadow-inner">
                          <span className="font-bold text-gray-700">你的答案: </span>
                          <span className="text-lg font-bold text-gray-600">{userAnswer}</span>
                        </div>
                      ...
                      {!showExplanation && (
                        <div className="text-center py-8">
                          ...
                          <p className="text-gray-600 font-medium">正在准备图解演示...</p>
                        </div>
                      )}

                      {/* Step-by-step Explanation */}
                      {showExplanation && explanationSteps.length > 0 && (
                        <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-4 border-purple-200 p-6 rounded-2xl shadow-lg">
