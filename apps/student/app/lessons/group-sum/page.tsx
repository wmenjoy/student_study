...
export default function GroupSumPage() {
  ...
  return (
    <LessonRunner title="枚举/配对：组数求和" skillId="math-group-sum" intro={{ story: "将数列配成和相同的组，快速求解总和。", goal: "会按目标和进行配对并统计对数", steps }} hints={groupScript.hints} variantGen={(diff)=>{
      ...
      else if(diff==="medium"){items.push({prompt:"随意写出一组配对",placeholder:"如 3+8",check:x=>{const s=x.replace(/ /g,'').split('+');if(s.length!==2)return false;const a=parseInt(s[0]),b=parseInt(s[1]);return a+b===t && arr.includes(a)&&arr.includes(b)}})}
      else {items.push({prompt:"写出所有配对（逗号分隔）",placeholder:"如 1+10,2+9,...",check:x=>{const ex=pairs().map(p=>p.join('+')).join(',');return x.replace(/ /g,'')===ex}})}
      return items
